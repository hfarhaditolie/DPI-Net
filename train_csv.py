"""
DPINet adapted for particle dynamics CSV data.
Uses the original DPINet architecture from models.py.
"""
...
ARGS = SimpleNamespace(
    ...
    attr_dim=1,         # [normalised_radius]
    relation_dim=1,     # dummy edge feature (all ones)
...
def compute_statistics(matrices):
    ...
    for mat in matrices:
        # mat: [10, T, N] → reshape to [T*N, dim]
        pos_acc.append(mat[:3].reshape(3, -1).T)
        vel_acc.append(mat[3:6].reshape(3, -1).T)
        rad_acc.append(mat[9].reshape(-1))

    all_pos = np.concatenate(pos_acc, axis=0)   # [total, 3]
    all_vel = np.concatenate(vel_acc, axis=0)   # [total, 3]
    all_rad = np.concatenate(rad_acc, axis=0)   # [total]
...
def build_relations(pos, k):
    """
    Build a KNN graph in DPINet's sparse-matrix format.

    Returns (Rr, Rs, Ra, node_r_idx, node_s_idx) or None if no edges.
    Rr / Rs are sparse FloatTensors of shape [N, n_rels].
    """
    N = pos.shape[0]
    tree = spatial.cKDTree(pos)
    _, nbrs = tree.query(pos, k=min(k + 1, N))  # k+1 includes self

    pairs = [(i, int(j)) for i in range(N) for j in nbrs[i] if j != i]
    if not pairs:
        return None

    rels = np.array(pairs, dtype=np.int64)   # [n_rels, 2]
    n_rels = len(rels)
    ...
    return Rr, Rs, Ra, node_r_idx, node_s_idx
...
def prepare_step(mat_t, mat_t1, stat, radii_stat):
    """
    Build (attr, state, rels, N, label) tensors for one timestep transition.

    mat_t, mat_t1 : [10, N] numpy arrays for timesteps t and t+1.
    state  = [norm_pos, norm_vel]          shape [N, 6]
    attr   = [norm_radius]                 shape [N, 1]
    label  = normalised next velocity      shape [N, 3]
    """
    ...
    attr  = norm_radii[:, None].astype(np.float32)
    label = norm_next_vel.astype(np.float32)

    N    = pos.shape[0]
    rels = build_relations(pos, KNN_K)  # graph built on raw positions
...
def train(model, matrices, stat, radii_stat):
    ...
    for epoch in range(EPOCHS):
        ...
        for mat in matrices:
            ...
            for t in range(T - 1):
                attr_t, state_t, rels, N, label_t = prepare_step(
...
def inference(model, mat, times, stat, radii_stat):
    ...
    for t in tqdm(range(len(times) - 1)):
        ...
        norm_radii = (curr_radii - radii_stat[0]) / radii_stat[1]

        state_t = torch.FloatTensor(np.concatenate([norm_pos, norm_vel], axis=1))
        attr_t  = torch.FloatTensor(norm_radii[:, None])
        ...
        for p in next_pos:
            results.append({'time': times[t + 1], 'x': p[0], 'y': p[1], 'z': p[2]})
